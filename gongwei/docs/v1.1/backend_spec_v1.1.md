...
> 修訂：2026-08-16，更新 v1.0 前端／企劃引用，記錄 LINE Login Channel 已建立
> 修訂：2026-08-16，定稿正式網域、LINE Login v2.1、NPC CMS、玩家名冊與統一歷程；移除主線與人物關係
> 狀態：v1.1 後端開發基線，可直接建立 Solution、Migration、OpenAPI 與 Sprint Backlog
> 技術：.NET 10 LTS／ASP.NET Core 10 + PostgreSQL 17/18 + EF Core/Npgsql
> 玩家前台：GitHub Pages `https://miglow.vip/gongwei/`
...
v1.1 將領域規格拆成下列可直接開工的文件。舊版只供追溯，不得與 v1.1 混用：

| 文件 | 用途 | 規模 |
|---|---|---|
| 本文件 | 架構、領域規則、交易、狀態、開發順序 | 後端實作基準 |
| [README_v1.1.md](./backend-spec/README_v1.1.md) | 後端工程交付索引、建置順序、Bootstrap 與上線驗收 | 新環境開工入口 |
| [schema_v1.1.sql](./backend-spec/schema_v1.1.sql) | 可執行 PostgreSQL DDL | 60 Tables、FK、Check、Partial Unique Index、Trigger |
| [api_v1_v1.1.md](./backend-spec/api_v1_v1.1.md) | 完整 HTTP Contract 清單 | 234 個 Method + Path、權限、DTO、錯誤與交易要求 |
| [line_login_v1.1.md](./backend-spec/line_login_v1.1.md) | LINE OAuth/OIDC、PKCE、Session、CSRF、CORS 與測試矩陣 | 登入安全實作基準 |
| [rank_catalog_v1.1.md](./backend-spec/rank_catalog_v1.1.md) | 三類角色完整位號、晉降門檻、月俸與建角初值 | 規則權威來源 |
| [seed_rules_v1.1.sql](./backend-spec/seed_rules_v1.1.sql) | 可重複執行的位階、能力標籤、地圖與公開設定 Seed | 全位號與初始能力 |
| [seed_npcs_v1.1.sql](./backend-spec/seed_npcs_v1.1.sql) | 既有 8 位 NPC 初始發布資料與重繪立繪路徑 | 只新增缺少 Code，不覆蓋 CMS 編輯 |

`schema_v1.1.sql` 是資料庫結構的權威來源；EF Core Migration 產物必須與其語意一致。API 開發時需同步產生 OpenAPI，CI 比對 Breaking Change。

規格衝突時的優先順序為：本輪使用者確認事項 → 本文件的領域規則與狀態機 → `api_v1_v1.1.md` 的 HTTP Contract → `schema_v1.1.sql` 的儲存約束 → 原始 Obsidian 遊戲文件。若 DDL、OpenAPI 或程式行為與較高順位文件不同，必須先修正文檔與 Migration，不得在 Controller 內另藏例外規則。

### 0.1 v1.1 一致性修正

- LINE Developers 已建立 Provider「Max」下的 LINE Login Channel「宮闈浮生」，Channel ID `2011123657`；正式 Callback 固定為 `https://gongwei-api.miglow.vip/api/v1/auth/line/callback`。Channel Secret 只能由 IIS／Secret Store 注入。
- 玩家前端固定為 `https://miglow.vip/gongwei/`；API 使用 `https://gongwei-api.miglow.vip`，管理後台使用 `https://gongwei-admin.miglow.vip`，避免占用 Miglow 既有 `api.miglow.vip`。
- 取消主線故事模組：移除 StoryArc／Chapter／Node／ContentRevision 及全部 Story API，地圖改由 NPC 資訊與地點事件組成。
- 取消人物關係模組：移除 Relationship／History 及全部 Relationship API；親子關係 `offspring_links` 仍屬生育資料，保留。
- 新增 `line_login_attempts`、`npcs`、`npc_revisions`、`ability_label_definitions`、`character_progress`、`character_chronicle_entries`，資料表總數維持 60 張。
- 玩家名冊依模糊最後上線時間排序；角色今日／歷史歷程由不可變 Chronicle 統一供應，不再由前端拼湊 Ledger、Stats 與事件資料。
- 原位階文件的「主線次數」因主線模組取消，v1.1 統一映射為已完成且已結算事件次數 `settledEventCount`。
- 保留不限行動點、永久 Audit／事件文字／死亡資料、管理員調整需理由與 Audit、不設一般經濟雙人覆核門檻等已定稿規則。

### 0.2 v1.1 已定稿規則

- 建角欄位依原始人設單拆成正式欄位；Draft 可不完整，Submit 才驗證字數與年齡。宮妃 15～18 歲；皇嗣姓蕭、年齡 0。
- 四能力為體質、容貌、心計、福氣，採 0～1000；初始值只由獲准起始位號推導。威望、恩寵與銀兩皆從 0 開始。
- 能力標籤由後端依 Seed 回傳：例如體質 570 必須是「康健」；前端不得自行維護另一份門檻。
- 不建立主線劇情與人物關係功能；NPC 有可由管理後台編輯、審核、發布、封存及回復的獨立故事資料。
...
- 管理網頁的 NPC／地點／事件內容編輯、人物稱號授予及具版本的遊戲設定發布。

### 1.3 非目標

- 不做即時聊天室、WebSocket 同步戰鬥。
- 不讀取 LINE 群對話。
- 不做主線 StoryArc／Chapter／Node，也不做人物關係分數與關係歷史。
...
```text
玩家 ── HTTPS ── miglow.vip/gongwei/ ── GitHub Pages 玩家前台
                          │ Cookie + CSRF API Request
                          ▼
管理員 ─ HTTPS ─ gongwei-admin.miglow.vip ─┐
                                 ├─ IIS
玩家前台 ─────── gongwei-api.miglow.vip ───┤  ├─ GongWei.Api (ASP.NET Core)
...
正式管理後台只由 `gongwei-admin.miglow.vip` 的 IIS Site 提供，不部署於 GitHub Pages。PostgreSQL 與原始媒體目錄不得對公網開放。IIS 負責 HTTPS Binding 並透過 ASP.NET Core Module 啟動 ASP.NET Core 10；應用程式只接受已知 Proxy／IIS Integration 傳入的 Forwarded Headers。玩家圖片不可寫入 GitHub Pages、IIS Web Root 或資料庫 bytea；資料庫只保存 Storage Key、Hash、尺寸、裁切與審核狀態。
...
├─ tools/
│  └─ GongWei.AdminCli/         首次 Super Admin Bootstrap；僅伺服器本機執行
├─ tests/
...
- `gongwei-api.miglow.vip` 與 `gongwei-admin.miglow.vip` 各自設定 HTTPS Binding、有效憑證、HTTP→HTTPS、HSTS 與 Host Allowlist；Production 不開 IIS Directory Browsing，也不使用會接受任意 Host 的頂層萬用 Binding。
...
| `/Admin/Npcs` | CE／GM／SA；AUD 唯讀 | NPC 資料、故事、立繪、草稿、審核、發布、版本比較與回復 | NPC Commands／Queries |
...
### 2.5 LINE Login 與正式網域契約

| 項目 | 正式值 |
|---|---|
| 玩家前端 | `https://miglow.vip/gongwei/` |
| API Origin | `https://gongwei-api.miglow.vip` |
| API Base | `https://gongwei-api.miglow.vip/api/v1` |
| 管理後台 | `https://gongwei-admin.miglow.vip/` |
| LINE Callback | `https://gongwei-api.miglow.vip/api/v1/auth/line/callback` |
| LINE Channel ID | `2011123657` |
| LINE Scope | `openid profile`；不要求 Email |
| Production CORS Origin | `https://miglow.vip`；Origin 不含 `/gongwei/` |

LINE Login 使用 OAuth 2.0 Authorization Code + OpenID Connect v2.1，強制 State、Nonce 與 PKCE S256。`/auth/line/start` 建立 10 分鐘一次性 `line_login_attempts`；Callback 先鎖定並消耗 Attempt，再交換 Token、驗證 ID Token 的簽章、Issuer、Audience、Expiry 與 Nonce。只以 `sub` 作 LINE 帳號唯一識別，不保存 Access／Refresh Token。

玩家 Session 使用 256-bit Opaque Token，資料庫只存 Hash；Cookie 為 Host-only `gw_session; Secure; HttpOnly; SameSite=Lax; Path=/`，Idle 7 日、Absolute 30 日、每帳號最多 5 個有效 Session。SPA 跨 Origin Request 一律 `credentials: include`，寫入再驗證 Session-bound CSRF Token。完整流程、錯誤碼與環境設定見 [api_v1_v1.1.md](./backend-spec/api_v1_v1.1.md#21-line-login-v21-完整流程)。

LINE Channel 在 Developing 時僅 Admin／Tester 可用；正式開放一般玩家前必須切換 Published。Channel Secret、Data Protection Key、Session Secret、DB Password 不得寫入 Git、規格範例值、前端 Bundle 或 Log。
...
| 模組 | 核心資料 | 可依賴 | 不可反向依賴 |
|---|---|---|---|
| Identity | Users、LINE Login Attempts、Sessions、Admin Roles | Audit | Economy／Events 不得決定登入 |
| Characters | Application、Character、Portrait、Media、Title、Stats、Rank、Residence | Identity、Audit | 前端不能改角色狀態 |
| World/Content | World State、Locations、NPC、NPC Revision、Game Setting | Characters、Approval、Audit | 已發布內容不得無版本覆寫 |
| Events | Rooms、Participants、Posts、Results、Chronicle | Characters、Economy、Outbox | Economy 不依賴 Event UI |
...
| `line_login_attempts` | `state_hash` | LINE State/Nonce/PKCE 一次性、10 分鐘有效、Return URL Allowlist |
...
| `ability_label_definitions` | `(ability_code,min_value)` | 四能力區間、標籤唯一；570 體質解析為康健 |
| `character_progress` | `character_id` | 已結算事件、投稿、自戲與每週活躍計數 Projection |
...
| Table | 用途 | 最重要約束 |
|---|---|---|
| `world_state` | 年號、宮廷日、全域開關 | Singleton ID = 1；無主線章節 |
...
| `npcs` | NPC 公開資料、故事與立繪 | Code Unique、Draft／Review／Published／Archived |
| `npc_revisions` | NPC 建立／編輯／發布／回復快照 | Append-only、NPC Revision Unique |
| `character_chronicle_entries` | 玩家今日與歷史遊玩歷程 | Append-only、公開層級、數值與資源 Before/Delta/After Snapshot |
...
erDiagram
    ...
    CHARACTERS ||--|| CHARACTER_PROGRESS : tracks
    ABILITY_LABEL_DEFINITIONS }o--o{ CHARACTER_STATS : labels
    ...
    WORLD_LOCATIONS ||--o{ NPCS : locates
    NPCS ||--o{ NPC_REVISIONS : versions
    CHARACTERS ||--o{ CHARACTER_CHRONICLE_ENTRIES : records
    WORLD_LOCATIONS ||--o{ CHARACTER_CHRONICLE_ENTRIES : classifies
...
- Stats／Wallet／Inventory／Rank／Status 等玩家可見異動必須在同一交易新增 Chronicle Snapshot。
- 能力標籤區間不得重疊或留空；NPC／玩家 Response 均由後端解析標籤。
- LINE State 必須一次性消耗，Return URL 必須通過 `miglow.vip/gongwei/` Allowlist。
...
4. 建立 Character、CharacterStats、CharacterProgress、Wallet、StatusHistory 與第一筆 Chronicle。
...
預設模式為 `event_only`，代表系統不做每日隨機流產。執行 `miscarry` 還必須提供 `triggerCode`、至少 5 字理由，以及可驗證的 `sourceType/sourceId`，例如已結算事件或嚴重中毒 Status Effect；一般管理員不能只按按鈕無理由結束 Pregnancy。

可選流產模式範例：

| 模式 | 範例規則 | 特性 |
|---|---|---|
| `event_only`（預設建議） | 嚴重中毒事件成立、已發布事件結果或玩家審核通過的劇情選擇後才觸發；可設治療期限 | 最可控，不會因每日登入突然失去皇嗣 |
...
6. 寫 Chronicle、Outbox，保存 Idempotency Response，Commit。
...
- 鎖 EventRoom，再依 Character UUID 固定排序鎖 Stats、Wallet、Inventory 與 Progress。
- EventResult、Reward、Ledger、Inventory、Progress、Chronicle 與通知同一交易。
...
| 親子、出生、懷孕與 Chronicle 歷史 | 永久保留；既有親子關係不因死亡刪除 |
...
1. NPC 與遊戲設定先寫 Draft；玩家 API 永遠只讀 Published NPC 與 PublishedValue。
2. ContentEditor 編輯 NPC 時必須帶 `If-Match`；每次建立、編輯、送審、發布、封存、回復均新增不可變 `npc_revisions` Snapshot。
3. NPC 發布前驗證名稱、故事長度、站內或受信 HTTPS 立繪 URL、地點 FK 與 `public_profile` Schema。發布交易寫狀態、Publisher、時間、Audit 與 Outbox。
4. 回復舊 NPC 版本是「以舊 Snapshot 建立新 Revision」，不修改歷史列；不提供 NPC Hard Delete API。
...
### 6.14 玩家名冊、能力標籤與 Chronicle

- 玩家名冊只列每個帳號的目前角色，排序鍵固定為模糊化 `users.last_seen_at DESC, character_id`；不公開 LINE User ID、歷代角色連結、Session、IP、管理備註或死亡角色帳號關聯。
- API 每 5 分鐘最多更新一次 `users.last_seen_at` 與目前 Session `last_seen_at`，避免每個 Request 寫 DB；玩家看到的是 5 分鐘粒度標籤。
- 四能力標籤由 `ability_label_definitions` 解析並隨 DTO 回傳。體質 570 必須回 `康健`；0 體質回 `逝世`，其他能力 0 回 `未定`。
- 任何會改變 Stats、Prestige、Favor、Wallet、Inventory、Rank、Status、Pregnancy 或公開結果的成功交易，必須同交易新增 `character_chronicle_entries`，保存 Before、Delta、After、地點、來源、可見性與發生時間。
- `scope=today` 以 `Asia/Taipei` 的 00:00–24:00 查詢；`scope=all` 使用 `(happened_at,id)` Cursor。本人可看 Public + OwnerOnly，其他玩家只看 Public，管理員依 Policy 可看 AdminOnly。
- Chronicle 是歷程 Snapshot，不取代 Ledger、InventoryTransaction、RankHistory、StatusHistory、Birth、Death 或 Audit 等領域權威資料；兩者必須能以 Source Type/ID、Request ID 或 Transaction ID 互相追查。
...
- Cookie Session + CSRF；Production CORS 只允許 `https://miglow.vip` 與明確 Staging Origin。
...
3. Character Me/Public、玩家名冊、Chronicle、Title、World State/Map、NPC、Notifications。
...
7. NPC 基本編輯／審核／發布／Revision、Title Definition／Assignment、Game Setting Draft／Publish／Revision。
...
- NPC Publish／Restore、Title Grant／Revoke、GameSetting Publish／Restore。
...
| `monthly-stipend` | 宮廷月切換 | GameYearMonth + Character + Currency Domain Key |
| `session-cleanup` | 每小時 | 批次刪除已過保存期 Session |
| `line-login-attempt-cleanup` | 每小時 | 刪除已過期且超過安全保留窗的登入 Attempt |
...
- `gongwei-admin.miglow.vip` 不啟用 CORS，不接受來自 GitHub Pages 的管理操作；管理表單由 IIS 上的 ASP.NET Core Admin Web 同源送出。
- LINE Callback 僅接受一次性、未過期、未消耗的 State；強制 PKCE S256、Nonce 與 ID Token Claim 驗證。不得記錄 Authorization Code、State、Nonce、Verifier 或 Token。
...
- 玩家名冊 `users.last_seen_at + current character` Cursor 排序。
- Character Chronicle 今日／歷史 Cursor Feed。
- NPC Published 列表與地點篩選。
...
- EF Migration 與 `schema_v1.1.sql` 約束需一致；Raw SQL Migration 建立 Partial Index、Append-only Trigger 與跨表 Trigger。
...
| 類型 | 必測內容 |
|---|---|
| Domain Unit | 狀態機、容量、抽選、價格、效果、死亡規則、能力標籤、NPC 發布、設定 Schema |
| PostgreSQL Integration | DDL、FK/Check/Partial Unique、Trigger、Transaction、Deadlock Retry |
| API Contract | 234 支 Path、Auth Policy、DTO、Problem Details、OpenAPI Snapshot |
| Concurrency | 最後一個生育名額、最後庫存、同 Event 結算、同 Pregnancy 出生 |
| Security | LINE State/Nonce/PKCE、CSRF、CORS、IDOR、RBAC、自我覆核、Session 撤銷、Markdown XSS、Secret 禁止曝光、圖片偽裝／EXIF／解碼炸彈 |
...
- DDL/Migration、LINE Login Attempt/User/Session、OpenAPI、Problem Details、Request ID。
...
- World/Map、NPC Editor/Revision/Publish、Player Directory/Chronicle、Event Room/Post/Settlement。
...
- External Submission、Intrigue、Effect。
...
- Windows Server 版本、異地備份位置與最終 RPO/RTO；Web Host 已確定為 IIS。
- `gongwei-api.miglow.vip`、`gongwei-admin.miglow.vip` 的 DNS、TLS 憑證與 IIS Binding；網址本身已在本版定稿。
...
## 17. 後端交付 Definition of Done

每個 Sprint 的 Endpoint 只有在下列條件全部完成時才算交付：

1. Application／Domain 規則、Authorization Policy、Validator、Transaction 與 Audit 已實作，不只完成 Controller。
2. EF Core Migration 可從空白 PostgreSQL 建庫，並與 [schema_v1.1.sql](./backend-spec/schema_v1.1.sql) 的 FK、Check、Index、Trigger 語意一致。
3. OpenAPI 包含所有 Request／Response／Problem Details、Cookie、CSRF、Idempotency-Key、If-Match 與權限說明；實際 Path 數與 [api_v1_v1.1.md](./backend-spec/api_v1_v1.1.md) 對齊。
4. Unit、PostgreSQL Integration、API Contract、Authorization、Idempotency、Concurrency 與 Security Tests 通過。
5. 玩家可見異動同交易寫 Chronicle；管理寫入同交易寫 Audit；通知走 Outbox，不在交易內直接呼叫外部服務。
6. Production 設定不含開發 Callback、`AllowAnyOrigin`、測試 Secret、詳細錯誤或自動 Migration。
7. IIS Staging 通過 `/health/live`、`/health/ready`、LINE Admin/Tester 登入、Session Rotation、CSRF、CORS、圖片上傳與 App Pool 回收測試。
8. 部署文件包含 Migration、備份、回復、Data Protection Key、憑證更新及 Worker 安裝步驟。

## 18. 實作標準來源

- [LINE Login v2.1 Web App 整合](https://developers.line.biz/en/docs/line-login/integrate-line-login/)
- [LINE Login Security Checklist](https://developers.line.biz/en/docs/line-login/security-checklist/)
- [.NET 官方支援政策](https://dotnet.microsoft.com/en-us/platform/support/policy)
- [ASP.NET Core 10 發布至 IIS](https://learn.microsoft.com/en-us/aspnet/core/tutorials/publish-to-iis?view=aspnetcore-10.0)
