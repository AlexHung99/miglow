-- 宮闈浮生 PostgreSQL schema v1.1
-- Target: PostgreSQL 17/18
-- Canonical domains: https://miglow.vip/gongwei/ and https://gongwei-api.miglow.vip
-- v1.1 removes the cancelled main-story and relationship modules, and adds durable
-- LINE Login attempts, NPC content, ability labels, progress and unified chronicles.
...
-- One-time server-side correlation state for LINE Login v2.1/OIDC + PKCE.
-- Raw state, nonce and PKCE verifier are never stored. Protected payload is encrypted
-- with the ASP.NET Core Data Protection key ring shared by the API instances.
CREATE TABLE game.line_login_attempts (
    ...
    CHECK (return_url LIKE 'https://miglow.vip/gongwei/%' OR
           return_url = 'https://miglow.vip/gongwei/'),
    CHECK (consumed_at IS NULL OR consumed_at >= created_at)
);

CREATE INDEX ix_line_login_attempts_expiry
    ON game.line_login_attempts(expires_at)
...
CREATE TABLE game.admin_role_assignments (
    ...
    public_title  varchar(80),
    public_duty   varchar(500),
    ...
    CHECK (is_public = false OR public_display_name IS NOT NULL)
);
...
CREATE TABLE game.character_title_definitions (
    ...
    category            varchar(30) NOT NULL
                        CHECK (category IN ('rank', 'achievement', 'event', 'honorary', 'secret')),
...
CREATE TABLE game.ability_label_definitions (
    ability_code    varchar(20) NOT NULL
                    CHECK (ability_code IN ('vitality', 'appearance', 'strategy', 'luck')),
    min_value       smallint NOT NULL CHECK (min_value BETWEEN 0 AND 1000),
    max_value       smallint NOT NULL CHECK (max_value BETWEEN 0 AND 1000),
    ...
    updated_at      timestamptz NOT NULL DEFAULT now(),
    version         bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    PRIMARY KEY (ability_code, min_value),
    UNIQUE (ability_code, display_label),
    CHECK (min_value <= max_value)
);

-- Materialized counters used by rank evaluation. Source transactions and chronicles
-- remain authoritative; these counters must be changed only by Application handlers.
CREATE TABLE game.character_progress (
    character_id               uuid PRIMARY KEY REFERENCES game.characters(id) ON DELETE CASCADE,
    settled_event_count        bigint NOT NULL DEFAULT 0 CHECK (settled_event_count >= 0),
    approved_event_post_count  bigint NOT NULL DEFAULT 0 CHECK (approved_event_post_count >= 0),
    approved_external_play_count bigint NOT NULL DEFAULT 0 CHECK (approved_external_play_count >= 0),
    self_play_word_count       bigint NOT NULL DEFAULT 0 CHECK (self_play_word_count >= 0),
    week_start_date            date NOT NULL,
    weekly_message_count       integer NOT NULL DEFAULT 0 CHECK (weekly_message_count >= 0),
    updated_at                 timestamptz NOT NULL DEFAULT now(),
    version                    bigint NOT NULL DEFAULT 1 CHECK (version > 0)
);
...
CREATE TABLE game.npcs (
    ...
    sex                 varchar(10) CHECK (sex IN ('female', 'male', 'unknown')),
    summary             varchar(1500) NOT NULL DEFAULT '',
    story_markdown      text NOT NULL DEFAULT '',
    public_profile      jsonb NOT NULL DEFAULT '{}'::jsonb CHECK (jsonb_typeof(public_profile) = 'object'),
    portrait_asset_id   uuid REFERENCES game.media_assets(id) ON DELETE SET NULL,
    portrait_url        text,
    primary_location_id uuid REFERENCES game.world_locations(id) ON DELETE SET NULL,
    ...
    version             bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    CHECK (char_length(btrim(display_name)) BETWEEN 1 AND 100),
    CHECK (char_length(story_markdown) <= 50000),
    CHECK (portrait_asset_id IS NOT NULL OR portrait_url IS NOT NULL),
    CHECK ((status = 'published' AND published_by IS NOT NULL AND published_at IS NOT NULL)
           OR status <> 'published')
);

CREATE INDEX ix_npcs_player_list
    ON game.npcs(status, sort_order, display_name)
    WHERE status = 'published';

CREATE TABLE game.npc_revisions (
    id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    npc_id          uuid NOT NULL REFERENCES game.npcs(id) ON DELETE RESTRICT,
    revision_no     integer NOT NULL CHECK (revision_no > 0),
    snapshot        jsonb NOT NULL CHECK (jsonb_typeof(snapshot) = 'object'),
    ...
    changed_by      uuid NOT NULL REFERENCES game.users(id) ON DELETE RESTRICT,
    changed_at      timestamptz NOT NULL DEFAULT now(),
    UNIQUE (npc_id, revision_no)
);

-- Unified player-facing history. Every successful operation that changes a visible
-- value writes one immutable row in the same transaction as its source operation.
CREATE TABLE game.character_chronicle_entries (
    id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    character_id        uuid NOT NULL REFERENCES game.characters(id) ON DELETE RESTRICT,
    entry_type          varchar(30) NOT NULL
                        CHECK (entry_type IN ('event', 'economy', 'inventory', 'rank', 'status',
                                              'reproduction', 'intrigue', 'admin', 'system')),
    visibility          varchar(20) NOT NULL DEFAULT 'public'
                        CHECK (visibility IN ('public', 'owner_only', 'admin_only')),
    title               varchar(150) NOT NULL,
    detail              varchar(3000) NOT NULL DEFAULT '',
    location_id         uuid REFERENCES game.world_locations(id) ON DELETE SET NULL,
    source_type         varchar(60) NOT NULL,
    source_id           uuid,
    stat_changes        jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (jsonb_typeof(stat_changes) = 'array'),
    resource_changes    jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (jsonb_typeof(resource_changes) = 'array'),
    happened_at         timestamptz NOT NULL,
    created_at          timestamptz NOT NULL DEFAULT now(),
    created_by          uuid REFERENCES game.users(id) ON DELETE SET NULL,
    request_id          varchar(80),
    metadata            jsonb NOT NULL DEFAULT '{}'::jsonb CHECK (jsonb_typeof(metadata) = 'object')
);

CREATE INDEX ix_character_chronicle_character_time
    ON game.character_chronicle_entries(character_id, happened_at DESC, id DESC);

CREATE INDEX ix_character_chronicle_public_time
    ON game.character_chronicle_entries(happened_at DESC, id DESC)
...
CREATE TABLE game.event_rooms (
    ...
    event_type          varchar(30) NOT NULL CHECK (event_type IN ('ritual', 'social', 'investigation',
