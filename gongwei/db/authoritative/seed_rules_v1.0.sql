-- 宮闈浮生規則種子資料 v1.0
-- Prerequisite: schema_v1.0.sql
-- Source details and confirmed 400 annual stipend correction: rank_catalog_v1.0.md
...
SELECT
    ...
    jsonb_build_object('source', 'rank_catalog_v1.0') AS promotion_rules
...
CREATE TEMP TABLE requirement_seed (
    applies_to_role varchar(20), display_name text,
    vitality integer, appearance integer, strategy integer, luck integer,
    mainline_count integer, weekly_messages integer, self_play_words integer
) ON COMMIT DROP;
...
SET promotion_rules = jsonb_strip_nulls(jsonb_build_object(
    'source','rank_catalog_v1.0',
    'vitality',q.vitality,'appearance',q.appearance,'strategy',q.strategy,'luck',q.luck,
    'mainlineCount',q.mainline_count,'weeklyMessages',q.weekly_messages,
...
SELECT
    ...
    '{"enabled":true,"url":null,"label":"請我們喝杯咖啡"}'::jsonb, NULL,
